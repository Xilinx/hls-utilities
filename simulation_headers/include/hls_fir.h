...
private:
    ...
    void gen_ip_inst()
    {
        ...
        int diff = CONFIG_T::input_length % CEIL(1./CONFIG_T::sample_period);
        if (diff) {
            printf("Warning: in a FIR instance, ceil(1/sample_period) must divide evenly input_length, but %d %% %d == %d. Synthesis will fail!\n", 
                CONFIG_T::input_length, CEIL(1./CONFIG_T::sample_period), diff);
        }
    ...
    void insert_fpga_ip() {
#ifdef AESL_SYN
static_assert((CONFIG_T::input_length % CEIL(1./CONFIG_T::sample_period) == 0),
  "Error: in a FIR instance, ceil(1/sample_period) must divide evenly input_length");
        #pragma HLS inline
    ...
    public:
        ...
        )
        {
            ...
            #pragma HLS array_reshape cyclic variable=in_V factor=CEIL(1./CONFIG_T::sample_period)
            #pragma HLS array_reshape cyclic variable=out_V factor=CEIL(1./CONFIG_T::sample_period)

            insert_fpga_ip();
            for (int i = 0; i < CONFIG_T::output_length; ++i) {
            #pragma HLS unroll factor=CEIL(1./CONFIG_T::sample_period)
                out_V[i] = in_V[i];
            }
        ...
        )
        {
            ...
            for (unsigned int i = 0; i < CONFIG_T::output_length * CONFIG_T::num_channels; i++) 
            ...
            config_t config_V[CONFIG_T::num_channels])
        {
            ...
            if (dummy)
                for (int i = 0; i < CONFIG_T::num_channels * CONFIG_T::output_length; ++i) {
            #pragma HLS unroll factor=CEIL(1./CONFIG_T::sample_period)
                    out_V[i] = in_V[i];
                }
        }
            ...
            config_t config_V[CONFIG_T::num_channels])
        {
            ...
            #pragma HLS dataflow
            in_data_t in_V1[CONFIG_T::input_length * CONFIG_T::num_channels];
            #pragma HLS array_reshape cyclic variable=in_V1 factor=CEIL(1./CONFIG_T::sample_period)
